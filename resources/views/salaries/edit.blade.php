        ...
        <form action="{{ route('salaries.update', $salary->id) }}" method="POST">
            ...
            <table>
                    ...
                    <td>
                        <select name="karyawan_id" id="karyawan_id">
                            @foreach ($employees as $employee)
                                <option value="{{ $employee->id }}" {{ $salary->karyawan_id == $employee->id ? 'selected' : '' }}>{{ $employee->nama_lengkap }} ({{ $employee->position->nama_jabatan ?? 'Tidak ada posisi' }})</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
                <tr>
                    ...
                    <td><input type="text" id="bulan" name="bulan" value="{{ old('bulan', $salary->bulan) }}"></td>
                </tr>
                <tr>
                    <td><label for="gaji_pokok">Gaji pokok:</label></td>
                    <td><input type="number" id="gaji_pokok" name="gaji_pokok" step="0.01" value="{{ old('gaji_pokok', $salary->gaji_pokok) }}" readonly></td>
                </tr>
                <tr>
                    <td><label for="tunjangan">Tunjangan:</label></td>
                    <td><input type="text" id="tunjangan" name="tunjangan" value="{{ old('tunjangan', $salary->tunjangan) }}"></td>
                </tr>
                <tr>
                    <td><label for="potongan">Potongan:</label></td>
                    <td><input type="text" id="potongan" name="potongan" value="{{ old('potongan', $salary->potongan) }}"></td>
                </tr>
                <tr>
                    <td><label for="total_gaji">Total Gaji:</label></td>
                    <td><input type="number" id="total_gaji" name="total_gaji" step="0.01" value="{{ old('total_gaji', $salary->total_gaji) }}"  readonly></td>
                </tr>
                <tr>
